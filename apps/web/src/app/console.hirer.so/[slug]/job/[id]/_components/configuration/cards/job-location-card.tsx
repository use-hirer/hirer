...
const jobLocationSchema = z.object({
  location: z.string().min(1, { message: "Please use at least 1 character." }),
});

type JobLocationFormValues = z.infer<typeof jobLocationSchema>;

interface JobLocationCardProps {
  location: string;
}

const JobLocationCard: React.FC<JobLocationCardProps> = ({ location }) => {
  const params = useParams() as { slug: string; id: string };
  const updateName = api.job.updateName.useMutation();
  const utils = api.useUtils();
  const [loading, setLoading] = useState(false);

  const form = useForm<JobLocationFormValues>({
  ...
  async function onSubmit(data: JobLocationFormValues) {
    setLoading(true);
  ...
  return (
    <Card className="rounded-md border-neutral-200 flex-grow-0 shadow-sm">
      <CardHeader>
        ...
        <CardDescription>
          Location of the job position being advertised.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
